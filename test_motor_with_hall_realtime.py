...
rospy.init_node('motor_with_hall', anonymous=True)
...
'''
        ...
        # 스크립트 업로드
        write_cmd = f"cat > /tmp/motor_with_hall.py << 'ENDOFFILE'\n{motor_script}\nENDOFFILE"
        ...
        chmod_cmd = "chmod +x /tmp/motor_with_hall.py"
        run_command(client, chmod_cmd, timeout=2)
        
        # 모터 실행 시작 (백그라운드)
        print("모터 시작...")
        motor_cmd = f"{env_cmd} && python2 /tmp/motor_with_hall.py > /tmp/motor_with_hall.log 2>&1 &"
                ...
                if current_message:
                    ...
                    if data:
                        ...
                        if changes:
                            ...
                            # 처음 몇 개와 주기적으로만 표시
                            if sample_count <= 5 or elapsed % 2.0 < 0.1:
                                print(f"{elapsed:7.2f}    {str(m0_hall):11s}  {str(m0_count):12s}  {str(m1_hall):11s}  {str(m1_count):12s}  {change_str}")
        ...
        if total_changes > 10:
            ...
            print(f"   ✅ 총 {total_changes}회의 변화가 감지되었습니다!")
        elif total_changes > 0:
            print(f"   ⚠️  홀 센서 값이 일부 변화했습니다 ({total_changes}회)")
            print(f"   ⚠️  모터가 느리게 회전하거나 홀 센서 감도 문제일 수 있습니다")
        else:
            print(f"   ❌ 홀 센서 값이 변하지 않았습니다.")
            print(f"   ❌ 모터가 실제로 회전하지 않았거나 홀 센서 연결 문제일 수 있습니다")
