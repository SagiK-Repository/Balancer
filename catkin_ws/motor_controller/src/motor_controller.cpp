...
MotorController::MotorController(ros::NodeHandle& nh, ros::NodeHandle& pnh)
    ...
    // 타이머 설정
    timer_ = nh_.createTimer(ros::Duration(0.000005), &MotorController::timerCallback, this);
...
void MotorController::checkHallSensor() {
    ...
    if (hall_state && !prev_hall_state_) {
        ...
        if(hall_count_%5 == 0){
            ...
            // 속도 계산 (RPM)
            if (duration > 0.000001) { // 1000us 이상의 간격
                float rpm = 60.0 / (duration*10/5); // 1분당 회전수 (RPM)
